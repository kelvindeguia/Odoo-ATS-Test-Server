...
class Department(models.Model):
    ...
    x_requestor = fields.Selection([('effy bongco', 'Effy Bongco'),
                                    ('zack concepcion', 'Zack Concepcion'),
                                    ('emir delos santos', 'Emir Delos Santos'),
                                    ('mariebien custodio', 'Mariebien Custodio'),
                                    ('thea vivar', 'Thea Vivar'),
                                    ('lovely_villafranca', 'Lovely Villafranca'),
                                    ('grace_lu', 'Grace Lu'),
                                    ('tamara bautista', 'Tamara Bautista'),
                                    ('peter lovell', 'Peter Lovell'),
                                    ('lisa ward', 'Lisa Ward'),
                                    ('support hiring', 'Support Hiring')],
                                   string="Requestor", store=True)
    ...
    @api.onchange('name')
    def _onchange_fields_update_department_name(self):
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                #     new_log = self.env['hr.updatelogs'].create({
                #         'x_log_record_name': record.name,
                #         'x_log_old_value': record._origin.name,
                #         'x_log_new_value': record.name,
                #         'client_id': record.id,
                #         # 'x_log_date': datetime.today(),
                #     })
                
    @api.onchange('x_category')
    def _onchange_fields_update_category(self):
    ...
    def _onchange_fields_update_requestor(self):
