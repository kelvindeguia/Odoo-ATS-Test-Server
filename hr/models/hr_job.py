...
class Job(models.Model):
    ...
    x_requestor = fields.Selection(
        [('effy bongco', 'Effy Bongco'), ('mariebien custodio', 'Mariebien Custodio'), (
            'thea vivar', 'Thea Vivar'), ('tamara bautista', 'Tamara Bautista'), ('peter lovell', 'Peter Lovell')],
    ...
    @api.onchange('name')
    def _onchange_fields_update_job_position(self):
    ...
    def _onchange_fields_update_client_name(self):
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                # Department ID
                if self.department_id != self._origin.department_id:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Department ID: {self._origin.department_id} to {self.department_id}\n"
    ...
    def _onchange_fields_update_website_published(self):
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.website_published != self._origin.website_published:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Is Published: {self._origin.website_published} to {self.website_published}\n"


    @api.onchange('description')
    def _onchange_fields_update_description(self):
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.description != self._origin.description:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Description: {self._origin.description} to {self.description}\n"


    @api.onchange('website_description')
    def _onchange_fields_update_website_description(self):
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.website_description != self._origin.website_description:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Website Description: {self._origin.website_description} to {self.website_description}\n"
