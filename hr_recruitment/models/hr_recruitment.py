...
class Applicant(models.Model):
    ...
    active = fields.Boolean("Active", default=True, groups='hr_recruitment.group_hr_recruitment_manager,hr_recruitment.group_hr_recruitment_delete_access',
                            help="If the active field is set to false, it will allow you to hide the case without removing it.")
    ...
    partner_name = fields.Char("Applicant's Name", tracking=True, default='Blase Pogi')
    ...
    x_recruitment_manager = fields.Selection(
        [('clarisse liberato', 'Clarisse Liberato'), ('rodney ynares', 'Rodney Ynares'),
         ('camille inovio', 'Camille Inovio'), ('alekcie vergara', 'Alekcie Vergara'),
         ('cesar jules van cayabat', 'Cesar Jules Van Cayabat')],
    ...
    x_requisition_id = fields.Many2one('hr.requisition', string='Requisition ID', domain="[('x_requisition_status','=','closed')]", store=True)
    ...
    x_application_category = fields.Selection(
        [('active', 'Active'), ('failed', 'Failed'), ('pooling', 'Pooling'), ('inactive', 'Inactive'),
         ('job_offer', 'Job Offer'), ('neo', 'NEO')],
        'Application Category', default='active',
        store=True)
    x_application_stage_ownership = fields.Selection([('recruitment', 'Recruitment'), ('client', 'Client'), ('sourcing', 'Sourcing')],
                                                     'Application Stage Ownership', store=True)
    ...
    # def onchange_x_requisition_id(self):
    #     for record in self:
    #         record.x_requisition_id = record.requisition_identifier_id
    ...
    # Update Logs
    @api.onchange('sourcer_id','partner_name','email_from','resume_file_name','x_mobile_number','x_secondary_number','type_id','linkedin_profile','indeed_profile','jobstreet_profile','x_app_source_category','x_app_specific_source_last','x_referrer_name','user_id','x_application_stage_ownership','x_application_category','stage_id','x_status_dropdown','x_date_received','x_date_dispatched','x_forecasted_start_date')
...
class Requisition(models.Model):
    ...
    applicant_ids = fields.One2many('hr.applicant', 'x_requisition_id', 'Applicants', compute='_compute_applicants', store=True)
    ...
    days_to_fill = fields.Integer('Days to Fill', compute="_compute_difference", store=True)
    ...
    test_field_2 = fields.Char('Test Field 2', store=True)
    
    # SLA Fields
    date_source = fields.Datetime('Date Source', store=True)
    end_date = fields.Datetime('End Date', store=True)
    days_ageing = fields.Integer('Days Ageing', compute="_compute_ageing_days")
    ...
    def _onchange_industry(self):
        for record in self:
            if record.x_industry != 'sales':
                record.client_website = 'Sales'
    ...
    @api.depends('x_requisition_status', 'x_job_classification', 'sourcing_date')
    ...
    @api.depends('x_requisition_status', 'x_job_classification', 'date_source')
    def _compute_days_to_fill(self):
        for record in self:
            sourcing_date = record.date_source
            if record.x_requisition_status == 'filled' or record.x_requisition_status == 'closed':
                record.date_filled = record.x_date_today
                filled_date = record.date_filled
                if record.date_filled:
                    date_difference = filled_date - sourcing_date
                    days_aeging = 0
                    for i in range(date_difference.days + 1):
                        date = filled_date + timedelta(days=i)
                        if date.weekday() < 5:  # Monday to Friday (0-4)
    ...
    api.depends('x_date_today')
    def _compute_ageing_days(self):
        for record in self:
            if not record.date_source:
                record.days_ageing = 0
            elif record.x_requisition_status == 'pending' and record.date_source:
                sourcing_date = record.date_source
                today_date = record.x_date_today
                date_difference = (today_date - sourcing_date).days
                days_ageing = 0
                for i in range(abs(date_difference) + 1):
                    date = sourcing_date + timedelta(days=i)
                    if date.weekday() < 5:  # Monday to Friday (0-4)
                        days_ageing += 1
                record.days_ageing = days_ageing
            else:
                record.days_ageing = 0

    # @api.onchange('x_requisition_status')
    # def onchange_x_requisition_status(self):
    #     for record in self:
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.x_department_id != self._origin.x_department_id:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Client Name: {self._origin.x_department_id} to {self.x_department_id}\n"
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.x_job_name != self._origin.x_job_name:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Job Title: {self._origin.x_job_name} to {self.x_job_name}\n"
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.x_calibration_call_availability != self._origin.x_calibration_call_availability:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Calibration Availability: {self._origin.x_calibration_call_availability} to {self.x_calibration_call_availability}\n"
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.client_website != self._origin.client_website:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Client Website: {self._origin.client_website} to {self.client_website}\n"
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.x_requisition_status != self._origin.x_requisition_status:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Requistion Status: {self._origin.x_requisition_status} to {self.x_requisition_status}\n"
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.date_filled != self._origin.date_filled:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Date Filled: {self._origin.date_filled} to {self.date_filled}\n"
        ...
        for record in self:
            ...
            if self._origin:  # check old value
                ...
                if self.x_date_opened != self._origin.x_date_opened:
                    self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Ongoing Sourcing Date: {self._origin.x_date_opened} to {self.x_date_opened}\n"

    

    # @api.onchange('x_job_name', 'x_department_id', 'x_calibration_notes', 'x_recruitment_requestor', 'x_calibration_call_availability',
    ...
    #               'x_hiring_manager', 'x_hiring_manager_email', 'x_recruitment_manager', 'x_assigned_recruiter')
    ...
    #         # Assigned Recruiter
    #         if self.x_assigned_recruiter != self._origin.x_assigned_recruiter:
    #             self.update_logs += f"{user_time:%m-%d-%Y %I:%M%p} | Assigned Recruiter: {self._origin.x_assigned_recruiter} to {self.x_assigned_recruiter}\n"                         
    ...
    def _compute_url(self):
        # menu_id = self._context.get('menu_id', False)
        # base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        base_url = 'https://isw-hub.com/web#id=%d&cids=1&menu_id=182&action=290&model=%s&view_type=form' % (
        self.id, self._name)
    ...
    def _compute_difference(self):
        for rec in self:
            ...
            if rec.date_source:
                ...
                # x_today = pd.to_datetime(x_today.astimezone(pytz.timezone('Asia/Manila')), format="%Y-%m-%d").date()
                x_date_create = pd.to_datetime(rec.date_source.astimezone(pytz.timezone('Asia/Manila')),
                                           format="%Y-%m-%d").date()
                ...
                if rec.x_requisition_status == 'recalibrate':
                    ...
                    rec.x_req_sla = False
                    rec.x_req_sla_total = False
                    rec.x_onhold_days = False
                    rec.x_onhold_start_date = False
                    rec.x_onhold_end_date = False
                    rec.x_old_onhold_checker = False
                    rec.x_onhold_counter = False
                    rec.date_source = False
                ...
                if rec.x_requisition_status == 'on hold':
                    ...
                    if rec.date_onhold != False and rec.x_onhold_end_date != False and rec.x_onhold_days > 0:
                        # END DATE HERE IS THE DATE REMOVED FROM BEING ON HOLD
                        rec.client_website = "True"
                ...
                #     rec.x_old_onhold_checker = 0
                ...
                elif rec.x_requisition_status == 'cancelled':
                    # rec.x_onhold_start_date = rec.x_date_today
                    ...
                    rec.date_filled = rec.x_date_today
                    rec.days_to_fill = rec.x_req_sla
                    var_onhold_start_date = rec.date_filled
                    var_onhold_start_date = pd.to_datetime(var_onhold_start_date.astimezone(pytz.timezone('Asia/Manila')),
                                                        format="%Y-%m-%d").date()
                    var_onhold_end_date = datetime.today()
                    var_onhold_end_date = pd.to_datetime(var_onhold_end_date.astimezone(pytz.timezone('Asia/Manila')),
            ...
            rec.x_req_sla_total = rec.x_req_sla - rec.x_onhold_days
            rec.x_req_sla = rec.x_req_sla_total
...
class Referral(models.Model):
    ...
    referral_logs_ids = fields.One2many('hr.logs', 'related_record_id', 'Meetings')
    

    @api.onchange('name', 'status')
    def _name_onchange_logs(self):
        for record in self:
            if record.name != record._origin.name:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.name,
                    'nv_char': record.name,
                    'related_record_id': record.id,
                    'updated_field': 'Candidate Name'
                }) 
            
    @api.onchange('email')
    def _email_onchange_logs(self):
        for record in self:
            if record.email != record._origin.email:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.email,
                    'nv_char': record.email,
                    'related_record_id': record.id,
                    'updated_field': 'Candidate Email Address'
                })  

    @api.onchange('mobile_number')
    def _mobile_number_onchange_logs(self):
        for record in self:
            if record.mobile_number != record._origin.mobile_number:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.mobile_number,
                    'nv_char': record.mobile_number,
                    'related_record_id': record.id,
                    'updated_field': 'Candidate Mobile Number'
                }) 

    @api.onchange('desired_position')
    def _desired_position_onchange_logs(self):
        for record in self:
            if record.desired_position != record._origin.desired_position:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.desired_position,
                    'nv_char': record.desired_position,
                    'related_record_id': record.id,
                    'updated_field': "Referral's Desired Position"
                })

    @api.onchange('employee_email')
    def _employee_email_onchange_logs(self):
        for record in self:
            if record.employee_email != record._origin.employee_email:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.employee_email,
                    'nv_char': record.employee_email,
                    'related_record_id': record.id,
                    'updated_field': 'Employee Email'
                })

    @api.onchange('emp_id')
    def _emp_id_onchange_logs(self):
        for record in self:
            if record.emp_id != record._origin.emp_id:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.emp_id,
                    'nv_char': record.emp_id,
                    'related_record_id': record.id,
                    'updated_field': 'Employee ID'
                })

    @api.onchange('emp_name')
    def _emp_name_onchange_logs(self):
        for record in self:
            if record.emp_name != record._origin.emp_name:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.emp_name,
                    'nv_char': record.emp_name,
                    'related_record_id': record.id,
                    'updated_field': 'Employee Name'
                })

    @api.onchange('emp_account')
    def _emp_account_onchange_logs(self):
        for record in self:
            if record.emp_account != record._origin.emp_account:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.emp_account,
                    'nv_char': record.emp_account,
                    'related_record_id': record.id,
                    'updated_field': 'Employee Account/Program/Department'
                })

    @api.onchange('requisition_id')
    def _requisition_id_onchange_logs(self):
        for record in self:
            if record.requisition_id != record._origin.requisition_id:
                new_log = self.env['hr.logs'].create({
                    'ov_char': record._origin.requisition_id.x_req_id,
                    'nv_char': record.requisition_id.x_req_id,
                    'related_record_id': record.id,
    ...
    # _sql_constraints = [
    #     ('mobile_number_uniq', 'unique (mobile_number)', "Mobile Numeber already exists !"),
    # ]
    # @api.depends('status')
...
class Pooling(models.Model):
    ...
    # Transfer to job portal function
...
class Logs(models.Model):
    ...
    related_record_id = fields.Many2one('hr.referral', 'Related Record', store=True)     
    priority = fields.Char('Priority', store=True)
...
class UpdateLog(models.Model):
    ...
    def _compute_x_log_document_id(self):
        ...
        # if self.x_department_id != self._origin.x_department_id:
        #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Client Name:  {self._origin.x_department_id} to {self.x_department_id}\n"

        #     user_time_naive = user_time.replace(tzinfo=None)

        
        #     # Create a log entry record in the hr.updatelogs
        #     if log_entry:
        #         log_data = {
        #             'x_log_name': self.x_department_id,
        #             'x_log_record_name': self.name,
        #             'x_log_old_value': self._origin.x_department_id,
        #             'x_log_new_value': self.x_department_id,
        #             'x_log_date': user_time_naive,
        #             'x_log_user_id': self.env.user.x_req_id,
        #             'x_log_value': self.x_req_id,
        #             'x_log_document_model': self._name,
        #             'x_log_field_change': 'Client Name',
        #         }
        #         self.env['hr.updatelogs'].create(log_data)
        ...
        # # Job Title
        # if self.x_job_name != self._origin.x_job_name:
        #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Job Title:  {self._origin.x_job_name} to {self.x_job_name}\n"
        ...
        #     if log_entry:
        #         log_data = {
        #             'x_log_name': self.x_job_name,
        #             'x_log_record_name': self.name,
        #             'x_log_old_value': self._origin.x_job_name,
        #             'x_log_new_value': self.x_job_name,
        #             'x_log_date': user_time_naive,
        #             'x_log_user_id': self.env.user.x_req_id,
        #             'x_log_value': self.x_req_id,
        #             'x_log_document_model': self._name,
        #             'x_log_field_change': 'Job Title',
        #         }
        #         self.env['hr.updatelogs'].create(log_data)
        ...
        # # Job Description
        # if self.x_job_description != self._origin.x_job_description:
        #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Job Description:  {self._origin.x_job_description} to {self.x_job_description}\n"
        ...
        #     if log_entry:
        #         log_data = {
        #             'x_log_name': self.x_job_description,
        #             'x_log_record_name': self.name,
        #             'x_log_old_value': self._origin.x_job_description,
        #             'x_log_new_value': self.x_job_description,
        #             'x_log_date': user_time_naive,
        #             'x_log_user_id': self.env.user.x_req_id,
        #             'x_log_value': self.x_req_id,
        #             'x_log_document_model': self._name,
        #             'x_log_field_change': 'Job Description',
        #         }
        #         self.env['hr.updatelogs'].create(log_data)        
        ...
        # # Calibration Notes
        # if self.x_calibration_notes != self._origin.x_calibration_notes:
        #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Calibration Notes:  {self._origin.x_calibration_notes} to {self.x_calibration_notes}\n"
        ...
        #     if log_entry:
        #         log_data = {
        #             'x_log_name': self.x_calibration_notes,
        #             'x_log_record_name': self.name,
        #             'x_log_old_value': self._origin.x_calibration_notes,
        #             'x_log_new_value': self.x_calibration_notes,
        #             'x_log_date': user_time_naive,
        #             'x_log_user_id': self.env.user.x_req_id,
        #             'x_log_value': self.x_req_id,
        #             'x_log_document_model': self._name,
        #             'x_log_field_change': 'Calibration Notes',
        #         }
        #         self.env['hr.updatelogs'].create(log_data)         
        ...
        # # Company
        # if self.company != self._origin.company:
        #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Company:  {self._origin.company} to {self.company}\n"
        ...
        #     if log_entry:
        #         log_data = {
        #             'x_log_name': self.company,
        #             'x_log_record_name': self.name,
        #             'x_log_old_value': self._origin.company,
        #             'x_log_new_value': self.company,
        #             'x_log_date': user_time_naive,
        #             'x_log_user_id': self.env.user.x_req_id,
        #             'x_log_value': self.x_req_id,
        #             'x_log_document_model': self._name,
        #             'x_log_field_change': 'Company',
        #         }
        #         self.env['hr.updatelogs'].create(log_data)      
        ...
        # # Career Level
        # if self.career_level != self._origin.career_level:
        #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Career Level:  {self._origin.career_level} to {self.career_level}\n"
        ...
        #     if log_entry:
        #         log_data = {
        #             'x_log_name': self.career_level,
        #             'x_log_record_name': self.name,
        #             'x_log_old_value': self._origin.career_level,
        #             'x_log_new_value': self.career_level,
        #             'x_log_date': user_time_naive,
        #             'x_log_user_id': self.env.user.x_req_id,
        #             'x_log_value': self.x_req_id,
        #             'x_log_document_model': self._name,
        #             'x_log_field_change': 'Career Level',
        #         }
        #         self.env['hr.updatelogs'].create(log_data)             
